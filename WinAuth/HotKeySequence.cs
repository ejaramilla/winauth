...
namespace WinAuth
{
    ...
    public class HoyKeySequence
    {
        ...
        public HoyKeySequence(string data)
        {
            ...
            {
                var match = Regex.Match(data, @"([0-9a-fA-F]{8})([0-9a-fA-F]{4})\t([^\t]*)\t(Y|N)(.*)",
                    RegexOptions.Multiline);
                if (match.Success)
                {
                    Modifiers = (WinAPI.KeyModifiers) BitConverter.ToInt32(
                        Authenticator.StringToByteArray(match.Groups[1].Value), 0);
                    HotKey = (WinAPI.VirtualKeyCode) BitConverter.ToUInt16(
                        Authenticator.StringToByteArray(match.Groups[2].Value), 0);
                    WindowTitle = match.Groups[3].Value;
                    Advanced = match.Groups[4].Value == "Y";
                    if (Advanced) AdvancedScript = match.Groups[5].Value;
                }
            }
        }
        ...
        public HoyKeySequence(XmlNode autoLoginNode, string password, decimal version)
        {
            ...
            if (node != null && node.InnerText.Length != 0)
                Modifiers = (WinAPI.KeyModifiers) BitConverter.ToInt32(Authenticator.StringToByteArray(node.InnerText),
                    0);
            node = autoLoginNode.SelectSingleNode("hotkey");
            if (node != null && node.InnerText.Length != 0)
                HotKey = (WinAPI.VirtualKeyCode) BitConverter.ToUInt16(Authenticator.StringToByteArray(node.InnerText),
                    0);
            node = autoLoginNode.SelectSingleNode("windowtitle");
            if (node != null && node.InnerText.Length != 0) WindowTitle = node.InnerText;
            node = autoLoginNode.SelectSingleNode("windowtitleregex");
            if (node != null && bool.TryParse(node.InnerText, out boolVal)) WindowTitleRegex = boolVal;
            node = autoLoginNode.SelectSingleNode("processname");
            if (node != null && node.InnerText.Length != 0) ProcessName = node.InnerText;
            node = autoLoginNode.SelectSingleNode("advanced");
            if (node != null && bool.TryParse(node.InnerText, out boolVal)) Advanced = boolVal;
                        ...
                        switch (encryptedType)
                        {
                            ...
                            case 'y':
                            {
                                ...
                                data = Authenticator.Decrypt(data, password,
                                    version >= (decimal) 1.7); // changed encrypted in 1.7
