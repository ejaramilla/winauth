...
namespace WinAuth
{
    ...
    public class KeyboardHook : IDisposable
    {
        ...
        public void UnHook()
        {
            for (var i = 0; i < m_hooked.Count; i++) WinAPI.UnregisterHotKey(m_form.Handle, i + 1);
            m_hooked.Clear();
        }
        ...
        public static bool IsHotkeyAvailable(Form form, Keys key, WinAPI.KeyModifiers modifier)
        {
            ...
            if (available) WinAPI.UnregisterHotKey(form.Handle, 0);
            return available;
        }
        ...
        public void KeyCallback(KeyboardHookEventArgs kea)
        {
            ...
            // call user events
            if (KeyPressed != null) KeyPressed(this, kea);
        }
    ...
    public class KeyboardSender
    {
        ...
        public void SendKeys(WinAuthForm form, string keys, string code)
        {
                            ...
                            if (cmdMatch.Success)
                            {
                                ...
                                switch (cmd)
                                {
                                    ...
                                    case "ENTER":
                                        ...
                                        break;
                                    case "WAIT":
                                        for (; repeat > 0; repeat--) Thread.Sleep(delay);
                                        break;
                                    case "COPY":
            ...
            {
                // resume input
                if (blocked) WinAPI.BlockInput(false);
                Application.DoEvents();
            }
        }
        ...
        private static IntPtr FindWindow(string window)
        {
            ...
            if (string.IsNullOrEmpty(window)) return WinAPI.GetForegroundWindow();
            ...
            if (match.Success)
            {
                ...
                if (match.Groups[2].Value.Contains("i")) regoptions |= RegexOptions.IgnoreCase;
    ...
    public class KeyboardHookEventArgs : EventArgs
    {
        ...
        public KeyboardHookEventArgs(Keys keyCode)
        {
            ...
            try
            {
                ...
                if (Alt) Modifiers |= WinAPI.KeyModifiers.Alt;
                if (Control) Modifiers |= WinAPI.KeyModifiers.Control;
                if (Shift) Modifiers |= WinAPI.KeyModifiers.Shift;
            }
            catch (Exception)
            {
            }
