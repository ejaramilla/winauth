...
namespace WinAuth
{
    ...
    public partial class ShowSteamTradesForm : ResourceForm
    {
        ...
        public SteamAuthenticator AuthenticatorData =>
            Authenticator != null ? Authenticator.AuthenticatorData as SteamAuthenticator : null;
        ...
        private async void confirmAllButton_Click(object sender, EventArgs e)
        {
            ...
            if (WinAuthForm.ConfirmDialog(this, "This will CONFIRM all your current trade confirmations." +
            ...
            try
            {
                ...
                {
                    if (cancelComfirmAll.IsCancellationRequested) break;

                    var start = DateTime.Now;

                    var result = await AcceptTrade(tradeIds[i]);
                    if (result == false || cancelComfirmAll.IsCancellationRequested) break;
                    if (i != 0)
                    {
        ...
        private async void cancelAllButton_Click(object sender, EventArgs e)
        {
            ...
            if (WinAuthForm.ConfirmDialog(this, "This will CANCEL all your current trade confirmations." +
                                                Environment.NewLine + Environment.NewLine +
                                                "Are you sure you want to continue?",
                MessageBoxButtons.YesNo, MessageBoxIcon.Warning, MessageBoxDefaultButton.Button2) != DialogResult.Yes)
                return;
            ...
            try
            {
                ...
                for (var i = tradeIds.Length - 1; i >= 0; i--)
                {
                    if (cancelCancelAll.IsCancellationRequested) break;

                    var start = DateTime.Now;

                    var result = await RejectTrade(tradeIds[i]);
                    if (result == false || cancelCancelAll.IsCancellationRequested) break;
                    if (i != 0)
                    {
                        var duration = (int) DateTime.Now.Subtract(start).TotalMilliseconds;
                        var delay = SteamClient.CONFIRMATION_EVENT_DELAY +
            ...
            do
            {
                ...
                try
                {
                        ...
                        if (steam.Login(username, password, captchaId, captchaText) == false)
                        {
                            ...
                            if (steam.Requires2FA)
                            {
                                WinAuthForm.ErrorDialog(this,
                                    "Invalid authenticator code: are you sure this is the current authenticator for your account?");
                                return;
                            }
                            ...
                            {
                                WinAuthForm.ErrorDialog(this,
                                    string.IsNullOrEmpty(steam.Error) == false
                                        ? steam.Error
                    ...
                    try
                    {
                        ...
                        if (string.IsNullOrEmpty(AuthenticatorData.SessionData) == false) Authenticator.MarkChanged();
                    }
                    ...
                    for (var row = 0; row < m_trades.Count; row++)
                    {
                        ...
                        label.Text = trade.Details + Environment.NewLine + trade.Traded + Environment.NewLine +
                                     trade.When;
                        label.Tag = trade.Id;
                ...
                {
                    Cursor.Current = cursor;
                    if (WinAuthForm.ErrorDialog(this, "An error occurred while loading trades", iere,
...
#endif
        {
            try
            {
                ...
                if (trade == null) throw new ApplicationException("Invalid trade");
...
#endif
                if (result == false) throw new ApplicationException("Trade cannot be confirmed");
...
#endif
                if (result == false) throw new ApplicationException("Trade cannot be cancelled");
        ...
        {
            if (control.Name.StartsWith(name) && control is T) return (T) control;
        ...
        private Control Clone(Control control, string suffix)
        {
            ...
            clone.SuspendLayout();
            if (clone is ISupportInitialize) ((ISupportInitialize) clone).BeginInit();

            // copy public properties
            foreach (var pi in type.GetProperties(BindingFlags.FlattenHierarchy | BindingFlags.Instance |
                                                  BindingFlags.Public))
            {
                if (pi.CanWrite == false || pi.CanRead == false) continue;
                if (pi.Name == "Controls" || pi.Name == "Name" || pi.Name == "WindowTarget") continue;
                ...
                if (value != null && value.GetType().IsValueType == false)
                {
                    ...
                    else if (value is ISerializable)
                    {
                        ...
                        if (newvalue.GetType() == value.GetType()) value = newvalue;
                    }
                }
            ...
            clone.ResumeLayout();
            if (clone is ISupportInitialize) ((ISupportInitialize) clone).EndInit();
        ...
        private void SetPolling()
        {
            ...
            if (m_loaded == false || pollAction.SelectedValue == null) return;
