...
namespace WinAuth
{
    ...
    internal class WinAuthHelper
    {
        ...
        public static WinAuthConfig LoadConfig(Form form, string configFile, string password = null)
        {
            ...
            if (string.IsNullOrEmpty(password) == false) config.Password = password;

            if (string.IsNullOrEmpty(configFile))
            {
                // check for file in current directory
                configFile = Path.Combine(Environment.CurrentDirectory, DEFAULT_AUTHENTICATOR_FILE_NAME);
                if (File.Exists(configFile) == false) configFile = null;
            }

            if (string.IsNullOrEmpty(configFile))
            {
                // check for file in exe directory
                configFile = Path.Combine(Path.GetDirectoryName(Application.ExecutablePath),
                    DEFAULT_AUTHENTICATOR_FILE_NAME);
                if (File.Exists(configFile) == false) configFile = null;
            }
            ...
            if (string.IsNullOrEmpty(configFile))
            {
                ...
                if (File.Exists(configFile) == false) return config;
            }
            ...
            if (fi.Exists && fi.IsReadOnly) config.IsReadOnly = true;
            ...
            if (data.Length == 0 || data[0] == 0)
                ...
                if (File.Exists(configFile + ".bak"))
                {
                    ...
                    if (data.Length != 0 && data[0] != 0) File.WriteAllBytes(configFile, data);
                }
            ...
            if (config.Version < WinAuthConfig.CURRENTVERSION)
            {
                ...
                foreach (var wa in config) wa.Created = fi.CreationTime;

                config.Upgraded = true;
            }

            if (changed && config.IsReadOnly == false) SaveConfig(config);
        ...
        public static string GetLastV2Config()
        {
            ...
            try
            {
                using (var key = Registry.CurrentUser.OpenSubKey(WINAUTH2REGKEY, false))
                {
                    ...
                    if (key != null
                        && (lastfile =
                            key.GetValue(string.Format(CultureInfo.InvariantCulture, WINAUTHREGKEY_LASTFILE, 1),
                                null) as string) != null
            ...
            using (var ms = new MemoryStream())
            {
                ...
                if (!fi.Exists || !fi.IsReadOnly)
                    ...
                    try
                    {
                        ...
                        if (File.Exists(config.Filename)) File.Move(config.Filename, config.Filename + ".bak");
        ...
        {
            if (config == null || wa == null || wa.AuthenticatorData == null) return;

            using (var sha = Authenticator.SafeHasher("SHA256"))
            {
                // get a hash based on the authenticator key
                var authkey =
                    Convert.ToBase64String(sha.ComputeHash(Encoding.UTF8.GetBytes(wa.AuthenticatorData.SecretData)));
                ...
                using (var sw = new EncodedStringWriter(Encoding.UTF8))
                {
                    ...
                    var pgpkey = string.IsNullOrEmpty(config.PGPKey) == false ? config.PGPKey : WINAUTH_PGP_PUBLICKEY;
        ...
        public static string ReadBackupFromRegistry(WinAuthConfig config)
        {
            ...
            foreach (var name in config.ReadSettingKeys(WINAUTHREGKEY_BACKUP))
            {
                ...
                if (val != null) buffer.Append(name + "=" + Convert.ToString(val)).Append(Environment.NewLine);
            }
        ...
        /// <summary>
        ///     Import a file containing authenticators in the KeyUriFormat. The file might be plain text, encrypted zip or
        ///     encrypted pgp.
        /// </summary>
            ...
            do
            {
                ...
                if (string.Compare(Path.GetExtension(file), ".zip", true) == 0)
                {
                    using (var fs = new FileStream(file, FileMode.Open, FileAccess.Read))
                    {
                        ...
                        {
                            zip = new ZipFile(fs);
                            if (string.IsNullOrEmpty(password) == false) zip.Password = password;

                            var buffer = new byte[4096];
                            foreach (ZipEntry entry in zip)
                            {
                                if (entry.IsFile == false ||
                                    string.Compare(Path.GetExtension(entry.Name), ".txt", true) != 0) continue;
                        ...
                        catch (ZipException ex)
                        {
                            if (ex.Message.IndexOf("password") != -1)
                            {
                                ...
                                if (form.ShowDialog(parent) == DialogResult.Cancel) return null;
            ...
            try
            {
                using (var sr = new StringReader(lines.ToString()))
                {
                    ...
                    while ((line = sr.ReadLine()) != null)
                    {
                        ...
                        var qm = line.IndexOf("?");
                        if (hash != -1 && hash < qm) line = line.Substring(0, hash) + "%23" + line.Substring(hash + 1);

                        // parse and validate URI
                        var uri = new Uri(line);

                        // we only support "otpauth"
                        if (uri.Scheme != "otpauth") throw new ApplicationException("Import only supports otpauth://");
                        ...
                        else if (uri.Host == "hotp")
                        {
                            ...
                            if (string.IsNullOrEmpty(issuer) == false) auth.Issuer = issuer;
                        }
                        else // if (string.Compare(issuer, "Google", true) == 0)
                        {
                            ...
                            if (string.Compare(issuer, "Google", true) == 0)
                                issuer = string.Empty;
                            else if (string.IsNullOrEmpty(issuer) == false) auth.Issuer = issuer;
                        }

                        var period = 0;
                        int.TryParse(query["period"], out period);
                        if (period != 0) auth.Period = period;

                        var digits = 0;
                        int.TryParse(query["digits"], out digits);
                        if (digits != 0) auth.CodeDigits = digits;

                        Authenticator.HMACTypes hmactype;
                        if (Enum.TryParse(query["algorithm"], true, out hmactype)) auth.HMACType = hmactype;
                        //
                        if (label.Length != 0)
        ...
        public static void ExportAuthenticators(Form form, IList<WinAuthAuthenticator> authenticators, string file,
            string password, string pgpKey)
        {
            ...
            using (var ms = new MemoryStream())
            {
                using (var sw = new StreamWriter(ms))
                {
                    ...
                    // reprotect
                    foreach (var auth in unprotected) auth.AuthenticatorData.Protect();
                    ...
                    if (string.Compare(Path.GetExtension(file), ".zip", true) == 0)
                        using (var zip = new ZipOutputStream(new FileStream(file, FileMode.Create, FileAccess.Write)))
                        {
                            ...
                            var entry = new ZipEntry(
                                ZipEntry.CleanName(Path.GetFileNameWithoutExtension(file) + ".txt"));
        ...
        public static NameValueCollection ParseQueryString(string qs)
        {
            ...
            if (string.IsNullOrEmpty(qs)) return pairs;
            ...
            foreach (var p in qs.Split('&'))
            {
                ...
                if (string.IsNullOrEmpty(v) == false)
                {
                    ...
                    string newv;
                    while ((newv = Uri.UnescapeDataString(v)) != v) v = newv;
                }
        ...
        public static string[] ReadRegistryKeys(string keyname)
        {
            ...
            try
            {
                using (var key = basekey.OpenSubKey(subkey))
                {
                    ...
                    for (var i = 0; i < keys.Count; i++) keys[i] = keyname + "\\" + keys[i];
        ...
        ///     public key in ASCII "-----BEGIN PGP PUBLIC KEY BLOCK----- .. -----END PGP PUBLIC KEY
        ///     BLOCK-----" format
        /// </param>
        ...
        public static string PGPEncrypt(string plain, string armoredPublicKey)
        {
            ...
            using (var encryptedStream = new MemoryStream())
            {
                ...
                {
                    var pedg = new PgpEncryptedDataGenerator(SymmetricKeyAlgorithmTag.Cast5, true, new SecureRandom());
                    ...
                    using (var pedgStream = pedg.Open(armored, new byte[4096]))
                    {
                        ...
                        using (var pcdgStream = pcdg.Open(pedgStream))
                        {
                            ...
                            using (var encrypter = pldg.Open(pcdgStream, PgpLiteralData.Binary, "", data.Length,
                                DateTime.Now))
                            {
            ...
            using (var ms = new MemoryStream(Encoding.ASCII.GetBytes(armoredPrivateKey)))
            {
                using (var dis = PgpUtilities.GetDecoderStream(ms))
                {
                    ...
                    foreach (PgpSecretKey key in keyring.GetSecretKeys())
                        privateKeys.Add(key.KeyId,
                            key.ExtractPrivateKey(keyPassword != null ? keyPassword.ToCharArray() : null));
                }
            }
                ...
                using (var inputStream = new MemoryStream(cipher))
                {
                    using (var ais = new ArmoredInputStream(inputStream))
                    {
                        ...
                        if (message is PgpEncryptedDataList)
                            foreach (PgpPublicKeyEncryptedData pked in ((PgpEncryptedDataList) message)
                                .GetEncryptedDataObjects())
                                message = new PgpObjectFactory(pked.GetDataStream(privateKeys[pked.KeyId]))
                                    .NextPgpObject();
                        if (message is PgpCompressedData)
                            message =
                                new PgpObjectFactory(((PgpCompressedData) message).GetDataStream()).NextPgpObject();
