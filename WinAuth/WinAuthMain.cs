...
namespace WinAuth
{
    ...
    internal static class WinAuthMain
    {
        ...
        /// </summary>
#if BETA
        public const string WINAUTH_UPDATE_URL =
            "https://raw.githubusercontent.com/winauth/winauth/master/docs/current-beta-version.xml";
#else
    public const string WINAUTH_UPDATE_URL =
 "https://raw.githubusercontent.com/winauth/winauth/master/docs/current-version.xml";
#endif
        ...
        public static List<RegisteredAuthenticator> REGISTERED_AUTHENTICATORS = new List<RegisteredAuthenticator>
        {
            new RegisteredAuthenticator
            {
                ...
                Icon = "WinAuthIcon.png"
            },
            null,
            new RegisteredAuthenticator
            {
                Name = "Google", AuthenticatorType = RegisteredAuthenticator.AuthenticatorTypes.Google,
                Icon = "GoogleIcon.png"
            },
            new RegisteredAuthenticator
            {
        ...
        private static void Main()
        {
            try
            {
                ...
                fileTarget.Layout =
                    @"${longdate} ${assembly-version} ${logger} ${message} ${exception:format=tostring}";
            ...
            catch (TimeoutException)
            {
                ...
                foreach (var process in Process.GetProcesses())
                    if (process.ProcessName == APPLICATION_NAME)
                    {
                        ...
                        if (hwnd == (IntPtr) 0) hwnd = WinAPI.FindWindow(null, APPLICATION_TITLE);
        ...
        public static void LogException(Exception ex, bool silently = false)
        {
            ...
            try
            {
                ...
                if (silently == false)
                {
                    ...
                    report.TopMost = true;
                    if (_form != null && _form.Config != null) report.Config = _form.Config;
                    if (report.ShowDialog() == DialogResult.Cancel) Process.GetCurrentProcess().Kill();
                }
            }
            catch (Exception)
            {
