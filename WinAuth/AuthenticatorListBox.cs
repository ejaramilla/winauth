...
namespace WinAuth
{
    ...
    public class AuthenticatorListBox : ListBox
    {
        ...
        protected override void OnMouseDoubleClick(MouseEventArgs e)
        {
            ...
            if (item != null) DoubleClick(this, new AuthenticatorListDoubleClickEventArgs(item.Authenticator));
        }
        ...
        public void Tick(object sender, EventArgs e)
        {
            for (var index = 0; index < Items.Count; index++)
            {
                ...
                if (auth.AutoRefresh)
                {
                    ...
                    var tillUpdate = (int) Math.Round(auth.AuthenticatorData.ServerTime %
                        (auth.AuthenticatorData.Period * 1000L) / 1000L * (360M / auth.AuthenticatorData.Period));
                    ...
                    if (item.DisplayUntil != DateTime.MinValue)
                    {
                        ...
                        if (item.DisplayUntil <= DateTime.Now)
                        {
                            ...
                            if (item.Authenticator.AuthenticatorData != null &&
                                item.Authenticator.AuthenticatorData.PasswordType ==
                                Authenticator.PasswordTypes.Explicit) ProtectAuthenticator(item);
        ...
        protected override void OnMouseUp(MouseEventArgs e)
        {
                    ...
                    if (item.Authenticator.AutoRefresh == false && item.DisplayUntil < DateTime.Now
                                                                && new Rectangle(
                                                                        ...
                                                                        (hasvscroll
                                                                            ...
                                                                            : 0), y + MARGIN_TOP, ICON_WIDTH,
                                                                        ICON_HEIGHT)
                                                                    .Contains(e.Location))
                    {
                        if (UnprotectAuthenticator(item) == DialogResult.Cancel) return;
            ...
            if (_draggedItem != null) _draggedItem = null;
        }
        ...
        protected override void OnMouseMove(MouseEventArgs e)
        {
            ...
            if (e.Button == MouseButtons.Left && _mouseDownLocation != Point.Empty && Items.Count > 1)
            {
                ...
                if (dx > 2 || dy > 2)
                {
                    ...
                    var hasvscroll = Height < Items.Count * ItemHeight;
                    _draggedBitmap = new Bitmap(Width - (hasvscroll ? SystemInformation.VerticalScrollBarWidth : 0),
                        ItemHeight);
        ...
        protected override void OnDragDrop(DragEventArgs e)
        {
            ...
            if (item != null)
            {
                ...
                try
                {
                    ...
                    for (var i = 0; i < Items.Count; i++) (Items[i] as AuthenticatorListitem).Index = i;
                    ...
                    if (_lastDragTopIndex != null) TopIndex = _lastDragTopIndex.Value;
                }
                finally
                {
                    // resume painting
        ...
        private void setRenameTextboxLocation()
        {
            if (_renameTextbox != null && _renameTextbox.Visible)
            {
                ...
                if (item != null)
                {
                    ...
                    if (RenameTextbox.Location.Y != y) RenameTextbox.Location = new Point(RenameTextbox.Location.X, y);
                    Refresh();
                }
            }
        }
        ...
        public TextBox RenameTextbox
        {
            get
            {
                ...
                if (_renameTextbox == null)
                {
                    ...
                    _renameTextbox.Font =
                        new Font("Microsoft Sans Serif", 10F, FontStyle.Regular, GraphicsUnit.Point, 0);
        ...
        {
            if (save == false) RenameTextbox.Tag = null;
        ...
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private void _renameTextbox_KeyPress(object sender, KeyPressEventArgs e)
        {
            if (e.KeyChar == 27)
            {
        ...
        public AuthenticatorListitem CurrentItem
        {
            get
            {
                if (_currentItem == null && Items.Count != 0) _currentItem = (AuthenticatorListitem) Items[0];
                return _currentItem;
            }
        ...
        private DialogResult UnprotectAuthenticator(AuthenticatorListitem item, Screen screen = null)
        {
            ...
            if (result == DialogResult.OK) item.UnprotectCount++;
        ...
        private void ProtectAuthenticator(AuthenticatorListitem item)
        {
            ...
            item.UnprotectCount--;
            if (item.UnprotectCount > 0) return;

            // reprotect the authenticator
            var auth = item.Authenticator;
            if (auth.AuthenticatorData == null) return;
        ...
        private void loadContextMenuStrip()
        {
            ...
            foreach (var entry in WinAuthMain.AUTHENTICATOR_ICONS)
            {
                ...
                {
                    if (parentItem.Tag is ToolStripMenuItem) parentItem = parentItem.Tag as ToolStripMenuItem;
                    ...
                    subitem.Image = new Bitmap(Assembly.GetExecutingAssembly()
                        .GetManifestResourceStream("WinAuth.Resources." + iconfile));
        ...
        private void SetContextMenuItems()
        {
            ...
            if (item == null || (auth = item.Authenticator) == null || auth.AuthenticatorData == null) return;

            var labelitem =
                menu.Items.Cast<ToolStripItem>().Where(i => i.Name == "contextMenuItemName")
                    .FirstOrDefault() as ToolStripLabel;
            labelitem.Text = item.Authenticator.Name;
            if (auth.HotKey != null) labelitem.Text += " (" + auth.HotKey + ")";
            ...
            menuitem =
                menu.Items.Cast<ToolStripItem>().Where(i => i.Name == "showCodeMenuItem").FirstOrDefault() as
                    ToolStripMenuItem;
            ...
            //
            menuitem = menu.Items.Cast<ToolStripItem>().Where(i => i.Name == "showGoogleSecretMenuItem")
            ...
            //
            menuitem =
                menu.Items.Cast<ToolStripItem>().Where(i => i.Name == "iconMenuItem").FirstOrDefault() as
                    ToolStripMenuItem;
            var subitem =
                menuitem.DropDownItems.Cast<ToolStripItem>().Where(i => i.Name == "iconMenuItem_default")
                    .FirstOrDefault() as ToolStripMenuItem;
            ...
            menuitem =
                menu.Items.Cast<ToolStripItem>().Where(i => i.Name == "syncMenuItem").FirstOrDefault() as
                    ToolStripMenuItem;
            menuitem.Visible = !(auth.AuthenticatorData is HOTPAuthenticator);
        }
        ...
        private void ProcessMenu(ToolStripItem menuitem)
        {
            ...
            if (menuitem.Name == "setPasswordMenuItem")
            {
                ...
                if (wasprotected == DialogResult.Cancel) return;
                try
                {
                    // show the new password form
                    var form = new SetPasswordForm();
                    if (form.ShowDialog(Parent as Form) != DialogResult.OK) return;
                ...
                {
                    if (wasprotected == DialogResult.OK) ProtectAuthenticator(item);
                }
            }
            else if (menuitem.Name == "showCodeMenuItem")
            {
                ...
                if (item.DisplayUntil != DateTime.MinValue) ProtectAuthenticator(item);
                    ...
                    if (wasprotected != DialogResult.OK)
                    {
                        ...
                        if ((mainform.Config.PasswordType & Authenticator.PasswordTypes.Explicit) != 0)
                        {
                            ...
                            while (true)
                            {
                                ...
                                if (mainform.Config.IsPassword(checkform.Password)) break;
            ...
            {
                if (WinAuthForm.ConfirmDialog(Parent as Form, strings.DeleteAuthenticatorWarning,
        ...
        public string GetItemCode(AuthenticatorListitem item, Screen screen = null)
        {
            ...
            if (wasprotected == DialogResult.Cancel) return null;
                ...
                using (var g = CreateGraphics())
                {
                    using (var font = new Font(Font.FontFamily, FONT_SIZE, FontStyle.Regular))
                    {
                        foreach (var item in items.Where(i => i.AutoWidth == 0))
                        {
                            ...
                            item.AutoWidth = MARGIN_LEFT + ICON_WIDTH + ICON_MARGIN_RIGHT +
                                             (int) Math.Ceiling(labelsize.Width) + PIE_WIDTH + MARGIN_RIGHT;
                        }
                    }
                }
        ...
        protected void OnDrawItem(DrawItemEventArgs e, Rectangle cliprect)
        {
            ...
            using (var brush = new SolidBrush(e.ForeColor))
            {
                ...
                using (var font = new Font(e.Font.FontFamily, FONT_SIZE, FontStyle.Regular))
                {
                    ...
                    rect = new Rectangle(e.Bounds.X + 64, e.Bounds.Y + MARGIN_TOP, (int) labelsize.Height,
                        (int) labelsize.Width);
                    if (cliprect.IntersectsWith(rect))
                        e.Graphics.DrawString(label, font, brush,
                            new RectangleF(e.Bounds.X + MARGIN_LEFT + ICON_WIDTH + ICON_MARGIN_RIGHT,
                                e.Bounds.Y + MARGIN_TOP, labelMaxWidth, labelsize.Height));
                    ...
                    if (showCode)
                        try
                        {
                            ...
                            item.LastCode = code;
                            if (code != null && code.Length > 5) code = code.Insert(code.Length / 2, " ");
                        }
                    ...
                    if (auth.AutoRefresh)
                    {
                        using (var piebrush = new SolidBrush(SystemColors.ActiveCaption))
                        {
                            using (var piepen = new Pen(SystemColors.ActiveCaption))
                            {
                                ...
                                var tillUpdate = (int) Math.Round(auth.AuthenticatorData.ServerTime %
                                                                  (auth.AuthenticatorData.Period * 1000L) / 1000L *
                                                                  (360M / auth.AuthenticatorData.Period));
                                e.Graphics.DrawPie(piepen, e.Bounds.X + e.Bounds.Width - (MARGIN_RIGHT + ICON_WIDTH),
                                    e.Bounds.Y + MARGIN_TOP + PIE_MARGIN, PIE_WIDTH, PIE_HEIGHT, PIE_STARTANGLE,
                                    PIE_SWEEPANGLE);
                    ...
                    else
                    {
                        if (showCode)
                            using (var piebrush = new SolidBrush(SystemColors.ActiveCaption))
                            {
                                using (var piepen = new Pen(SystemColors.ActiveCaption))
                                {
                                    var tillUpdate = (int) (item.DisplayUntil.Subtract(DateTime.Now).TotalSeconds *
                                        360 / item.DisplayUntil.Subtract(item.LastUpdate).TotalSeconds);
                                    e.Graphics.DrawPie(piepen,
                                        e.Bounds.X + e.Bounds.Width - (MARGIN_RIGHT + ICON_WIDTH),
                                        e.Bounds.Y + MARGIN_TOP + PIE_MARGIN, PIE_WIDTH, PIE_HEIGHT, PIE_STARTANGLE,
                                        PIE_SWEEPANGLE);
                                    e.Graphics.FillPie(piebrush,
                                        e.Bounds.X + e.Bounds.Width - (MARGIN_RIGHT + ICON_WIDTH),
                                        e.Bounds.Y + MARGIN_TOP + PIE_MARGIN, PIE_WIDTH, PIE_HEIGHT, PIE_STARTANGLE,
                                        tillUpdate);
                                }
