...
namespace WinAuth
{
    ...
    public partial class UpdateCheckForm : ResourceForm
    {
        ...
        private void UpdateCheckForm_Load(object sender, EventArgs e)
        {
            ...
            autoDropdown.Items.Add(
                new UpdateIntervalItem {Name = "Every 2 Weeks", Interval = new TimeSpan(14, 0, 0, 0)});
            autoDropdown.Items.Add(new UpdateIntervalItem {Name = "Every Month", Interval = new TimeSpan(30, 0, 0, 0)});

            if (Updater == null) Updater = new WinAuthUpdater(Config);
        ...
        {
            if (autoCheckbox.Checked && autoDropdown.SelectedIndex < 0) autoDropdown.SelectedIndex = 0;
        }
        ...
        {
            if (autoDropdown.SelectedIndex >= 0) autoCheckbox.Checked = true;
        }
        ...
        private string GetHtmlText(string message, params string[] args)
        {
            ...
            return string.Format(
                "<html><head><style>body {{width:" +
                (versionInfoLabel.Width - versionInfoLabel.Margin.Horizontal - versionInfoLabel.Padding.Horizontal -
                 SystemInformation.VerticalScrollBarWidth) +
                "px;}} html,body,div,p,ul {{margin:0;padding:0;}} p {{padding:0; margin:0 0 8px 0;}} </style></head><body>{0}</body></html>",
                message);
        }
