...
namespace WinAuth
{
    public partial class SetShortcutKeyForm : ResourceForm
    {
        ...
        private void SetShortcutKeyForm_Load(object sender, EventArgs e)
        {
            ...
            foreach (WinAPI.VirtualKeyCode vk in Enum.GetValues(typeof(WinAPI.VirtualKeyCode)))
                if (vk >= WinAPI.VirtualKeyCode.VK_SPACE)
                {
                    ...
                    if (Hotkey != null && Hotkey.Key == vk) selected = item;
                }
        ...
        private void okButton_Click(object sender, EventArgs e)
        {
            ...
            if (key == 0)
                Hotkey = null;
            else if (Hotkey == null) Hotkey = new HotKey();

            if (Hotkey != null)
            {
                var modifiers = WinAPI.KeyModifiers.None;
                if (shiftToggle.Checked) modifiers |= WinAPI.KeyModifiers.Shift;
                if (ctrlToggle.Checked) modifiers |= WinAPI.KeyModifiers.Control;
                if (altToggle.Checked) modifiers |= WinAPI.KeyModifiers.Alt;
        ...
        private class KeyItem
        {
            ...
            /// </summary>
            public WinAPI.VirtualKeyCode Key { get; }
