    ...
    public abstract class Authenticator : ICloneable
    {
        ...
        public virtual string SecretData
        {
            get =>
                ...
                ByteArrayToString(SecretKey) + "\t" + CodeDigits + "\t" + HMACType + "\t" + Period;
            set
            {
                if (string.IsNullOrEmpty(value) == false)
                {
                    ...
                    {
                        int digits;
                        if (int.TryParse(parts[1], out digits)) CodeDigits = digits;
                    }

                    if (parts.Length > 2) HMACType = (HMACTypes) Enum.Parse(typeof(HMACTypes), parts[2]);
                    if (parts.Length > 3)
                    {
                        int period;
                        if (int.TryParse(parts[3], out period)) Period = period;
                    }
                }
                else
                {
        ...
        public static Authenticator ReadXmlv2(XmlReader reader, string password = null)
        {
            ...
            if (authenticator == null) authenticator = new BattleNetAuthenticator();
            ...
            while (reader.EOF == false)
                if (reader.IsStartElement())
                {
                    switch (reader.Name)
                    {
                        ...
                        default:
                            if (authenticator.ReadExtraXml(reader, reader.Name) == false) reader.Skip();
                            break;
                    }
                }
                else
        ...
        public static PasswordTypes DecodePasswordTypes(string passwordTypes)
        {
            ...
            if (string.IsNullOrEmpty(passwordTypes)) return passwordType;
        ...
        public static string EncodePasswordTypes(PasswordTypes passwordType)
        {
            ...
            if ((passwordType & PasswordTypes.Explicit) != 0) encryptedTypes.Append("y");
            if ((passwordType & PasswordTypes.User) != 0) encryptedTypes.Append("u");
            if ((passwordType & PasswordTypes.Machine) != 0) encryptedTypes.Append("m");
        ...
        public void WriteToWriter(XmlWriter writer)
        {
            ...
            if (string.IsNullOrEmpty(encrypted) == false) writer.WriteAttributeString("encrypted", encrypted);
        ...
        */
            ...
            bool decode = false)
        {
            ...
            using (var sha = SafeHasher("SHA256"))
            {
                ...
                if (string.Compare(comparehash, hash) != 0) throw new BadPasswordException();
            }
            ...
            bool decode = false)
        {
            try
            {
                ...
                if ((encryptedTypes & PasswordTypes.Explicit) != 0)
                {
                    ...
                    if (string.IsNullOrEmpty(password)) throw new EncryptedSecretDataException();
		...
		public static void TestPBKDF2()
		{
			...
			byte[] tv1 =
 new byte[] { 0x0c, 0x60, 0xc8, 0x0f, 0x96, 0x1f, 0x0e, 0x71, 0xf3, 0xa9, 0xb5, 0x24, 0xaf, 0x60, 0x12, 0x06, 0x2f, 0xe0, 0x37, 0xa6 };
			kg = new PBKDF2(Encoding.Default.GetBytes("password"), Encoding.Default.GetBytes("salt"), 1);
			DK = kg.GetBytes(20);
			Assert.AreEqual(DK, tv1);

			byte[] tv2 =
 new byte[] { 0xea, 0x6c, 0x01, 0x4d, 0xc7, 0x2d, 0x6f, 0x8c, 0xcd, 0x1e, 0xd9, 0x2a, 0xce, 0x1d, 0x41, 0xf0, 0xd8, 0xde, 0x89, 0x57 };
			kg = new PBKDF2(Encoding.Default.GetBytes("password"), Encoding.Default.GetBytes("salt"), 2);
			DK = kg.GetBytes(20);
			Assert.AreEqual(DK, tv2);

			byte[] tv3 =
 new byte[] { 0x4b, 0x00, 0x79, 0x01, 0xb7, 0x65, 0x48, 0x9a, 0xbe, 0xad, 0x49, 0xd9, 0x26, 0xf7, 0x21, 0xd0, 0x65, 0xa4, 0x29, 0xc1 };
			kg = new PBKDF2(Encoding.Default.GetBytes("password"), Encoding.Default.GetBytes("salt"), 4096);
			DK = kg.GetBytes(20);
			Assert.AreEqual(DK, tv3);

			byte[] tv4 =
 new byte[] { 0x2f, 0x25, 0x5b, 0x3a, 0x95, 0x46, 0x3c, 0x76, 0x62, 0x1f, 0x06, 0x80, 0xa2, 0xb3, 0x35, 0xad, 0x90, 0x3b, 0x85, 0xde };
