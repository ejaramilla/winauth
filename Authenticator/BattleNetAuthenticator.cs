    ...
    public class BattleNetAuthenticator : Authenticator
    {
        ...
        public void Enroll()
        {
                    ...
                    if (georesponse.StatusCode == HttpStatusCode.OK)
                        using (var ms = new MemoryStream())
                        {
                            using (var bs = georesponse.GetResponseStream())
                            {
                                ...
                                int read;
                                while ((read = bs.Read(temp, 0, RESPONSE_BUFFER_SIZE)) != 0) ms.Write(temp, 0, read);
            ...
            try
            {
                ...
                if (string.IsNullOrEmpty(configcountry) == false) country = configcountry;
            }
            ...
            try
            {
                ...
                if (string.IsNullOrEmpty(configregion) == false) region = configregion;
            }
            catch (InvalidOperationException)
            {
            }
            ...
            if (BitConverter.IsLittleEndian) Array.Reverse(serverTime);
            ...
            for (var i = oneTimePad.Length - 1; i >= 0; i--) secretKey[i] ^= oneTimePad[i];
            SecretKey = secretKey;
        ...
        public override void Sync()
        {
            ...
            try
            {
                ...
                // extract the server time
                if (BitConverter.IsLittleEndian) Array.Reverse(responseData);
                // get the difference between the server time and our current time
        ...
        public void Restore(string serial, string restoreCode)
        {
            ...
            for (var i = 0; i < 10; i++) restoreCodeBytes[i] = ConvertRestoreCodeCharToByte(arrayOfChar[i]);
        ...
        private static string GetMobileUrl(string region)
        {
            ...
            if (upperregion.Length > 2) upperregion = upperregion.Substring(0, 2);
        ...
        {
            // return if not set
            if (string.IsNullOrEmpty(Serial) || SecretKey == null) return string.Empty;
            ...
            for (var i = 0; i < 10; i++) code.Append(ConvertRestoreCodeByteToChar(digestdata[startpos + i]));
        ...
        private static string GeneralRandomModel()
        {
            ...
            for (var i = MODEL_SIZE; i > 0; i--) model.Append(MODEL_CHARS[random.Next(MODEL_CHARS.Length)]);
