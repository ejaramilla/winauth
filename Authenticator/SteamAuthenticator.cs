...
namespace WinAuth
{
    ...
    public class SteamAuthenticator : Authenticator
    {
        ...
        public override string SecretData
        {
            get
            {
                if (Client != null && Client.Session != null) SessionData = Client.Session.ToString();
            ...
            set
            {
                ...
                if (string.IsNullOrEmpty(value) == false)
                {
                    ...
                    if (string.IsNullOrEmpty(session) == false) SessionData = session;
                }
                else
                {
                    SecretKey = null;
        ...
        public SteamClient GetClient()
        {
            lock (this)
            {
                if (Client == null) Client = new SteamClient(this, SessionData);
        ...
        private string Request(string url, string method, NameValueCollection data = null,
            CookieContainer cookies = null, NameValueCollection headers = null, int timeout = 0)
        {
            ...
            request.Referer =
                COMMUNITY_BASE; // + "/mobilelogin?oauth_client_id=DE45CD61&oauth_scope=read_profile%20write_profile%20read_client%20write_client";
        ...
        public bool Enroll(EnrollState state)
        {
            ...
            try
            {
                ...
                if (string.IsNullOrEmpty(state.OAuthToken))
                {
                    ...
                    if (cookies.Count == 0)
                    {
                        cookies.Add(new Uri(COMMUNITY_BASE + "/"),
                            new Cookie("mobileClientVersion", "3067969+%282.1.3%29"));
                    ...
                    if (rsaresponse.SelectToken("success").Value<bool>() != true)
                        throw new InvalidEnrollResponseException("Cannot get steam information for user: " +
                                                                 state.Username);
                    ...
                    data.Add("emailsteamid",
                        state.EmailAuthText != null ? state.SteamId ?? string.Empty : string.Empty);
                    ...
                    if (loginresponse.ContainsKey("emailauth_needed") && (bool) loginresponse["emailauth_needed"])
                    {
                        if (loginresponse.ContainsKey("emaildomain"))
                        {
                            ...
                            if (string.IsNullOrEmpty(emaildomain) == false) state.EmailDomain = emaildomain;
                        }
                        ...
                        (bool) loginresponse["login_complete"] == false || loginresponse.ContainsKey("oauth") == false)
                    {
                        ...
                        if (loginresponse.ContainsKey("message")) state.Error = (string) loginresponse["message"];
                        return false;
                    }
                ...
                if (state.RequiresActivation == false)
                {
                    ...
                    if (response.IndexOf("status") == -1 &&
                        tfaresponse.SelectToken("response.status").Value<int>() == 84)
                    {
                        // invalid response
                        state.OAuthToken = null; // force new login
                        state.RequiresLogin = true;
                    ...
                    if (steamdata.SelectToken("steamid") == null) steamdata.Add("steamid", state.SteamId);
                    if (steamdata.SelectToken("steamguard_scheme") == null) steamdata.Add("steamguard_scheme", "2");
        ...
        public override void Sync()
        {
            ...
            if (SecretKey == null && EncryptedData != null) throw new EncryptedSecretDataException();

            // don't retry for 5 minutes
            if (_lastSyncError >= DateTime.Now.AddMinutes(0 - SYNC_ERROR_MINUTES)) return;
        ...
        protected override string CalculateCode(bool resyncTime = false, long interval = -1)
        {
            ...
            if (BitConverter.IsLittleEndian) Array.Reverse(codeIntervalArray);
        ...
        /// </summary>
        private class InvalidRequestException : ApplicationException
        {
        ...
        /// </summary>
        private class UnauthorisedRequestException : InvalidRequestException
        {
