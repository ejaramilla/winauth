...
namespace WinAuth
{
    ...
    public class SteamClient : IDisposable
    {
        ...
        /// </summary>
        private static readonly Regex _tradesRegex =
            new Regex("\"mobileconf_list_entry\"(.*?)>(.*?)\"mobileconf_list_entry_sep\"",
                RegexOptions.Singleline | RegexOptions.IgnoreCase);

        private static readonly Regex _tradeConfidRegex = new Regex(@"data-confid\s*=\s*""([^""]+)""",
            RegexOptions.Singleline | RegexOptions.IgnoreCase);

        private static readonly Regex _tradeKeyRegex = new Regex(@"data-key\s*=\s*""([^""]+)""",
            RegexOptions.Singleline | RegexOptions.IgnoreCase);

        private static readonly Regex _tradePlayerRegex =
            new Regex("\"mobileconf_list_entry_icon\"(.*?)src=\"([^\"]+)\"",
                RegexOptions.Singleline | RegexOptions.IgnoreCase);

        private static readonly Regex _tradeDetailsRegex =
            new Regex(
        ...
        public class ConfirmationPoller
        {
            ...
            public static ConfirmationPoller FromJSON(JToken tokens)
            {
                ...
                if (token != null) poller.Duration = token.Value<int>();
                token = tokens.SelectToken("action");
                if (token != null) poller.Action = (PollerAction) token.Value<int>();
                token = tokens.SelectToken("ids");
                if (token != null) poller.Ids = token.ToObject<List<string>>();
        ...
        public class SteamSession
        {
            ...
            private void FromJson(string json)
            {
                ...
                if (token != null) OAuthToken = token.Value<string>();
                ...
                if (token != null) Confirmations = ConfirmationPoller.FromJSON(token);
            }
        }
        ...
        public bool Login(string username, string password, string captchaId = null, string captchaText = null)
        {
            ...
            if (IsLoggedIn() == false)
            {
                ...
                data.Add("captcha_text",
                    string.IsNullOrEmpty(captchaText) == false ? captchaText : "enter above characters");
                ...
                if (loginresponse.ContainsKey("login_complete") == false ||
                    (bool) loginresponse["login_complete"] == false || loginresponse.ContainsKey("oauth") == false)
                {
                    InvalidLogin = true;
                    ...
                    if (loginresponse.ContainsKey("emailauth_needed") && (bool) loginresponse["emailauth_needed"])
                    {
                        if (loginresponse.ContainsKey("emaildomain"))
                        {
                            ...
                            if (string.IsNullOrEmpty(emaildomain) == false) EmailDomain = emaildomain;
                        }
                    ...
                    if (loginresponse.ContainsKey("message")) Error = (string) loginresponse["message"];
        ...
        public bool Refresh()
        {
            try
            {
                ...
                var cookieuri = new Uri(COMMUNITY_BASE + "/");
                Session.Cookies.Add(cookieuri,
                    new Cookie("steamLogin", Session.SteamId + "||" + token.Value<string>()));

                token = json.SelectToken("response.token_secure");
                if (token == null) return false;
                Session.Cookies.Add(cookieuri,
                    new Cookie("steamLoginSecure", Session.SteamId + "||" + token.Value<string>()));
        ...
        private bool UmqLogin()
        {
            ...
            if (loginresponse.ContainsKey("umqid"))
            {
                ...
                if (loginresponse.ContainsKey("message")) Session.MessageId = Convert.ToInt32(loginresponse["message"]);
        ...
        public delegate void
            ConfirmationErrorDelegate(object sender, string message, PollerAction action, Exception ex);
        ...
        public void PollConfirmations(ConfirmationPoller poller)
        {
            ...
            if (poller == null || poller.Duration <= 0) return;

            if (Session.Confirmations == null) Session.Confirmations = new ConfirmationPoller();
                ...
                while (!cancel.IsCancellationRequested && Session.Confirmations != null)
                {
                    ...
                    catch (Exception ex)
                    {
                        ...
                        if (retryCount >= ConfirmationPollerRetries)
                            ConfirmationErrorEvent(this, "Failed to read confirmations", Session.Confirmations.Action,
                                ex);
                        else
        ...
        public List<Confirmation> GetConfirmations()
        {
            ...
            while (match.Success)
            {
                ...
                if (innerMatch.Success) trade.Id = innerMatch.Groups[1].Value;
                innerMatch = _tradeKeyRegex.Match(tradeIds);
                if (innerMatch.Success) trade.Key = innerMatch.Groups[1].Value;
                ...
                {
                    if (innerMatch.Groups[1].Value.IndexOf("offline") != -1) trade.Offline = true;
            ...
            if (Session.Confirmations != null)
                ...
                {
                    if (Session.Confirmations.Ids == null) Session.Confirmations.Ids = new List<string>();
                    foreach (var conf in trades)
                    {
                        conf.IsNew = Session.Confirmations.Ids.Contains(conf.Id) == false;
                        if (conf.IsNew) Session.Confirmations.Ids.Add(conf.Id);
                    }
        ...
        public string GetConfirmationDetails(Confirmation trade)
        {
            ...
            if (JObject.Parse(response).SelectToken("success").Value<bool>())
            {
                ...
                var detailsRegex =
                    new Regex(@"(.*<body[^>]*>\s*<div\s+class=""[^""]+"">).*(</div>.*?</body>\s*</html>)",
                        RegexOptions.Singleline | RegexOptions.IgnoreCase);
                var match = detailsRegex.Match(ConfirmationsHtml);
                if (match.Success) return match.Groups[1].Value + html + match.Groups[2].Value;
            }
        ...
        {
            if (string.IsNullOrEmpty(Session.OAuthToken)) return false;
            ...
            try
            {
                ...
                if (Session.Confirmations != null)
                    lock (Session.Confirmations)
                    {
                        if (Session.Confirmations.Ids.Contains(id)) Session.Confirmations.Ids.Remove(id);
                    }
        ...
        private static string CreateTimeHash(long time, string tag, string secret)
        {
            ...
            var bufferSize = 8;
            if (string.IsNullOrEmpty(tag) == false) bufferSize += Math.Min(32, tag.Length);
        ...
        protected byte[] Request(string url, string method, NameValueCollection data, NameValueCollection headers)
        {
            ...
            lock (this)
            {
                ...
                if (string.Compare(method, "GET", true) == 0) url += (url.IndexOf("?") == -1 ? "?" : "&") + query;
        ...
        private static void LogException(string method, string url, CookieContainer cookies,
            NameValueCollection request, Exception ex)
        {
            ...
            Logger.Error(ex, "{0}\t{1}\t{2}", method, url, data);
        }
            ...
            string response)
        {
            ...
            Logger.Info("{0}\t{1}\t{2}\t{3}", method, url, data,
