...
namespace WinAuth
{
    ...
    public class Base32
    {
        ...
        protected Base32(string alphabet)
        {
            ...
            for (var i = 0; i < _digits.Length; i++) _map.Add(_digits[i], i);
        }
        ...
        public byte[] Decode(string encoded)
        {
            ...
            {
                if (_map.ContainsKey(c) == false) throw new Base32DecodingException("Illegal character: " + c);
                buffer <<= _shift;
                buffer |= _map[c] & _mask;
                bitsLeft += _shift;
                if (bitsLeft >= 8)
...
*/
    ...
    internal class Base32DecodingException : ApplicationException
    {
